...
async function updateStash(id, stashData) {
	
    const account = await getAccount(id);
    // copy params to account and save
    account.stash = stashData;
    account.updated = Date.now();
    await account.save();
...
async function getUrlHtml() {
    const axios = require('axios');
    const cheerio = require('cheerio');	  
	const url = 'https://www.pathofexile.com/trade/search/Harvest/ALXYtn';
	axios.get(url, {
      headers: {
	  ...
	  .then(response => {
        ...
        let listings = $('body').find('#trade').html();
